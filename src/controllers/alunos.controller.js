...
async function criarAluno(req, res, next) {
  try {
    ...
    if (!nome || typeof nome !== "string") {
      return res.status(400).json({ erro: "Campo 'nome' é obrigatório e deve ser um texto" });
    }

    const resultado = await pool.query(
      "INSERT INTO alunos (nome, data_nascimento, turma_id) VALUES ($1, $2, $3) RETURNING *",
...
async function deletarAluno(req, res, next) {
  try {
    ...
    const resultado = await pool.query("DELETE FROM alunos WHERE id = $1 RETURNING *", [id]);
